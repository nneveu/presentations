...
1,393 points \lsnote{did you mean `of' instead of `and the'}  BOBYQA optimization runs) were used to
...
simulations to reach the 25th generation\lsnote{107 core-hours}, and 64,000 simulations
to reach the 500th generation \lsnote{2,133 coure-hours}. 
...
acceptable results, with only the \lsnote{total simulation time} being 
the difference. 
\begin{figure}
...
by investigating novel optimization methods and comparing \lsnote{their} efficiency and precision. We will take advantage of the 
...
This multistart algorithm considers all results from previously evaluated simulations when determining where to start or continue a local optimization run \cite{jeff}. This allows for \lsnote{an} efficient 
