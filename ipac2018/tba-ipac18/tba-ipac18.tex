...
bunch to add to the others. A high power pulse is generated by 
...
powered by the PETS on the drive line. There is no other 
external power source. 

\section{Synchronization}
The kicker will ensure that one bunch train is supplied to 
each stage. This must happen in sync with the witness line.
The rf power generated in the drive line, must reach the 
accelerating structures as the witness beam is arriving.
...
Consider timing on the drive line; the bunch trains are generated
by a single pulse of the ultraviolet (UV) light that is split
several times in an set of optics called the multisplitter \cite{korea}.
The length of each UV pulse is \SI{769}{ps}, which is one wavelength.....  


\section{Kicker}
The kicker was specifically fabricated 
for this experiment. The initial design was adapted from 
work done at NIU \cite{kicker,korea}. The plates were lengthened
...
at \SI{8}{KV} RMS, and the other showing no discharge up to \SI{9}{KV}
...
While the general number of optics elements and their 
locations is known, there are still many free parameters that 
