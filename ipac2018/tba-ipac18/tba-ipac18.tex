...
\lsnote{I am confused about the following.  How can the laser pulse be one whole RF period?, beam would be spraying 
all over the place.  Did you mean the repetition rate of the laser is the same as the RF?  Also, why is the one period spacer 
between bunches needed?  Does this have to do with optimal filling of the decelerating structure?  Don't you need two 
bunch trains?  When exactly does the witness pulse go out in all of this : after both drive bunch trains?  Also, if you talk 
about times, you should be using period T, not wavelength lambda.}
To understand this timing, we must start with the UV laser.
Electron bunches in both the drive and witness line 
originate from the same ultraviolet (UV) laser pulse. A  network of UV optics and splitters 
deliver one laser pulse to the witness gun, and a pulse train to the drive gun \cite{korea}.
Each UV pulse is \SI{769}{ps} long, which is period (T)
...
This brings the length of an eight bunch train to 15~T or about \SI{11.5}{ns}.
Since only one bunch is sent to the witness line, the length of the 
laser pulse on the witness line is only T.
